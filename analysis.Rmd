...
ggplot(df_meta, aes(`SARS.CoV.2` , `SARS-CoV-2_ampl` / Primate_ampl, color=COVID, shape=Method)) + geom_point() + theme_classic() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ggtitle("Complete versus AmpliSeq") #  + geom_smooth(method='lm', se=FALSE)
...
## variancePartition for IL10RB expression
```{r vp.IL10RB, fig.height=3}
plotPercentBars(vp[geneInfo[Symbol=="IL10RB",Geneid],]) + theme(legend.position="right")
```
...
# get residuals just for IL10RB
```{r get.resids}

ensGene = geneInfo[Symbol=="IL10RB",Geneid]

df_IL10RB = data.frame(ID = colnames(resid), IL10RB = resid[ensGene,])
rownames(df_IL10RB) = c()

df = merge(df_IL10RB, info[,1:8], by='ID')

ggplot(df, aes(Gene, IL10RB)) + geom_point()

storeData( 'df_IL10RB', "Residualized expression of IL10RB (MDS1, MDS2, RIN)" )

ensGene = geneInfo[Symbol == "IL10RB", Geneid]
df_IL10RB = data.frame(ID = colnames(vobj), IL10RB = vobj$E[ensGene,])
storeData( 'df_IL10RB', "Observed expression of IL10RB" )

# IFNAR2
ensGene = geneInfo[Symbol=="IFNAR2",Geneid]

df_IFNAR2 = data.frame(ID = colnames(resid), IFNAR2 = resid[ensGene,])
rownames(df_IFNAR2) = c()

df = merge(df_IFNAR2, info[,1:8], by='ID')

ggplot(df, aes(Gene, IFNAR2)) + geom_point()

storeData( 'df_IFNAR2', "Residualized expression of IFNAR2 (MDS1, MDS2, RIN)" )

ensGene = geneInfo[Symbol == "IFNAR2", Geneid]
df_IFNAR2 = data.frame(ID = colnames(vobj), IFNAR2 = vobj$E[ensGene,])
storeData( 'df_IFNAR2', "Observed expression of IFNAR2" )
```
...
# gs.GO = get_GeneOntology(onto="MF")
# gsc.test = GeneSetCollection(c(gs.GO, gs.combine))
```
...
run_analysis = function(vobj, infoSub, baseline, coefs, form, gsc){
  ...
  res = run_zenith( fit, coefs, recodeToList(gsc), n_genes_min=10, n_genes_max=1000)
...
gheatmap = function( df_melt, row, col, value, label='', sort.row=TRUE, sort.col=TRUE, zlim=NULL, isSquare=TRUE){

  df_melt[[row]] = factor(df_melt[[row]], unique(df_melt[[row]]))
  df_melt[[col]] = factor(df_melt[[col]],  unique(df_melt[[col]]))

  df = matrix(NA, nlevels(df_melt[[col]]), nlevels(df_melt[[row]]), byrow=TRUE)
  colnames(df) = levels(df_melt[[row]])
  rownames(df) = levels(df_melt[[col]])

  tstat = with(df_melt, delta / se)

  for(i in 1:nrow(df_melt) ){
    df[ df_melt[[col]][i], df_melt[[row]][i]] = tstat[i]
  } 

  df_melt$Text = NA
  df_melt$Text[df_melt$FDR < 0.05] = '*'
  ...
  # substring(df_melt[[row]], 0, 80)
  fig =  ggplot(df_melt, aes_string(col, row, fill='value')) + geom_tile() + scale_fill_gradient2(name=label,low="blue", mid="white", high="red", limits=zlim) + theme_classic(8) + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + geom_text(aes(label=Text), hjust = 0.5, vjust=0.5)

  if( isSquare ){
    aspect.ratio = length(unique(df_melt[[row]])) / length(unique(df_melt[[col]]))
    fig = fig + theme(aspect.ratio=aspect.ratio)
  }

  attr(fig, 'row.names') = levels(df_melt[[row]])
  attr(fig, 'col.names') = levels(df_melt[[col]])
  fig
}
```
...
# CRISPRa
##########
keep.samples = with(info, Method == "CRISPRa")
keep <- filterByExpr(geneCounts[,keep.samples])
dge = DGEList( geneCounts[keep,keep.samples])
dge = calcNormFactors(dge)

infoSub.CRISPRa = droplevels(info[keep.samples,])

form = ~ Gene + COVID + MDS1 + MDS2 + RIN 
design = model.matrix( form, infoSub.CRISPRa)
vobj.CRIPSRa = voom(dge, design, plot=FALSE)

baseline = 'Scramble CRISPRa'
coefs.CRISPRa = c(paste0('Gene',c('IL10RB-1', 'IL10RB-2', 'IL10RB-3', 'IL10RB-4')), 'COVIDCoV+')
# coefs.CRISPRa = c(coefs.CRISPRa, 'GeneScramble shRNA')
res.CRISPRa = run_analysis( vobj.CRIPSRa, infoSub.CRISPRa, baseline, coefs.CRISPRa, form, gsc.test)

# shRNA
##########
keep.samples = with(info, Method == "shRNA")
keep <- filterByExpr(geneCounts[,keep.samples])
dge = DGEList( geneCounts[keep,keep.samples])
dge = calcNormFactors(dge)

infoSub.shRNA = droplevels(info[keep.samples,])

form = ~ Gene + COVID + MDS1 + MDS2 + RIN 
design = model.matrix( form, infoSub.shRNA)
vobj.shRNA = voom(dge, design, plot=FALSE)


baseline = 'Scramble shRNA'
coefs.shRNA = c(paste0('Gene',c('IFNAR2', 'IL10RB')), 'COVIDCoV+')
res.shRNA = run_analysis( vobj.shRNA, infoSub.shRNA, baseline, coefs.shRNA, form, gsc.test)
...
Compare each IL10RB pertubation to the Scramble CRISPRa baseline, including `~ MDS1 + MDS2 + RIN` as covariates. `expression il10RB ~ variable [perturbation vs baseline] + MDS1 + MDS2 + RIN`.
...
```{r other.de}
df_de = lapply(coefs.shRNA, function(coef){
...
df_de[with(df_de, coef=='COVIDCoV+'),] %>% dplyr::select(c('coef', 'Symbol', 'logFC', 't', 'P.Value')) %>% kable(caption="Effect of SARS-CoV-2 infection", row.names=FALSE) %>% kable_styling(full_width=FALSE)
```
...
lapply(coefs, function(x){
  ...
  tab = topTable(res$fit, coef=x, number=Inf)
  data.frame(shRNA = gsub("Gene", '', x), Count = sum(tab$adj.P.Val < 0.05))
}) %>% do.call(rbind, .) %>% kable(caption="# DE") %>% kable_styling(full_width=FALSE)
...
# Given C, a matrix of pairwise Jaccard similarities, 
#  remove one entry from a pair if simularity > cutoff
#  Continuous until all remaining similarity values are < cutoff
#  The entry of the selected pair is random
jaccard_filter = function(C, cutoff = .5){

  # set to zero, so diagonal is not the max value
  diag(C) = 0

  while(1){
    i = which.max(apply(C, 1, max))
    j = which.max(C[,i])
...
```{r show.zenith.CRISPRa, fig.width=8, fig.height=7}
library(metafor)

# merge CRIPSRa and shRNA 
res = data.table(rbind(res.CRISPRa$enrich, res.shRNA$enrich))

# do meta analysis for SARS-CoV-2 exposure
res_cov = res[Coef == 'COVIDCoV+',]
tab = res_cov[,table(Set)]

res_meta = lapply( names(tab[tab==2]), function(key){
  df = res_cov[Set==key,]

  tmp = suppressWarnings(rma(yi=delta, sei=se, data = df, method="FE"))

  data.frame( NGenes = max(df$NGenes), 
              Correlation = mean(df$Correlation),
              delta = tmp$beta,
              se = tmp$se,
              p.less = NA,
              p.greater = NA,
              PValue = tmp$pval,
              Direction = ifelse(sign(tmp$beta)==1, "Up", "Down"),
              FDR = NA,
              Set = key,
              Coef = 'COVIDCoV+')
})
res_meta = do.call(rbind, res_meta)
res_meta$FDR = p.adjust(res_meta$PValue, "fdr")

# remove original COVID results
res = res[Coef != 'COVIDCoV+',]

# now add meta-analysis
res = rbind(res, res_meta)

gs.any = unique(res[res$FDR < 0.05,]$Set)
# gs.any = unique(res[res$FDR < .1,]$Set)

resCombine = res[res$Set %in% gs.any,]
resCombine$Coef = factor(gsub("Gene", '', resCombine$Coef))
resCombine$value = with(resCombine, delta/se)
storeData( 'resCombine', "Gene set analysis of differential expression" )
resCombine$isGO = FALSE
resCombine$isGO[grep("GO", resCombine$Set)] = TRUE

# get similarity of gene sets based on jaccard index
df = data.table(resCombine[resCombine$isGO,])
df = df[,unique(data.table(Set, NGenes))]

# get similarity of gene sets based on jaccard index
M = pinnacle::getJaccardMatrix( gs.GO[unique(df$Set)] )

gs.keep = jaccard_filter( M, .2)
```
...
### CRISPR, all, GO
Show genesets that have FDR < 5% for any coefficient.  Stars indicate the coefficient where FDR < 5%.  Gene sets are filtered for redundancy using Jaccard index with a cutoff of 0.2.

```{r geneset.plots, fig.width=7, fig.height=16}
  
newLvls = c(
  'IL10RB-1' = "CRISPRa IL10RB-1",
  'IL10RB-2' = "CRISPRa IL10RB-2",
  'IL10RB-3' = "CRISPRa IL10RB-3",
  'IL10RB-4' = "CRISPRa IL10RB-4",
  'COVIDCoV+'= 'SARS-CoV-2 infection',
  'IL10RB'   = "shRNA IL10RB",
  'IFNAR2'   = "shRNA IFNAR2")
  
# set zlim
zlim = max(abs(resCombine$value))
zlim = c(-zlim, zlim)
   
figSigGO = resCombine %>% 
  mutate(Coef = plyr::revalue(Coef, newLvls)) %>% 
  mutate(Coef = factor(Coef, newLvls)) %>% 
  dplyr::filter(isGO) %>%    
  dplyr::filter(Set %in% gs.keep) %>%   
  gheatmap('Set', 'Coef', label="t-statistic", sort.col=FALSE, zlim=zlim)  
figSigGO         
```


### CRISPR, all, beta corona virus gene sets
```{r geneset.plots2, fig.width=7, fig.height=10}
figSigOther = resCombine %>% 
  mutate(Coef = plyr::revalue(Coef, newLvls)) %>% 
  mutate(Coef = factor(Coef, newLvls)) %>%  
  dplyr::filter(!isGO) %>%           
  gheatmap('Set', 'Coef', label="t-statistic", sort.col=FALSE, zlim=zlim)  
...
```{r cluster.signatures, eval=FALSE}
# cluster these responses.
df_tstat = merge(res.CRISPRa$df_tstat, res.shRNA$df_tstat, by="Geneid")
...
infoSub.CRISPRa.covneg = droplevels(info[keep.samples,])

form = ~ Gene + MDS1 + MDS2 + RIN 
design = model.matrix( form, infoSub.CRISPRa.covneg)
vobj.CRISPRa.covneg  = voom(dge, design, plot=FALSE)

baseline = 'Scramble CRISPRa'
coefs.CRISPRa = paste0('Gene',c('IL10RB-1', 'IL10RB-2', 'IL10RB-3', 'IL10RB-4'))
# coefs.CRISPRa = c(coefs.CRISPRa, 'GeneScramble shRNA')
res.CRISPRa = run_analysis( vobj.CRISPRa.covneg, infoSub.CRISPRa.covneg, baseline, coefs.CRISPRa, form, gsc.test)

# shRNA
###########
keep.samples = with(info, COVID=='CoV-' & Method == "shRNA")
keep <- filterByExpr(geneCounts[,keep.samples])
dge = DGEList( geneCounts[keep,keep.samples])
dge = calcNormFactors(dge)

infoSub.shRNA.covneg = droplevels(info[keep.samples,])

form = ~ Gene + MDS1 + MDS2 + RIN 
design = model.matrix( form, infoSub.shRNA.covneg)
vobj.shRNA.covneg  = voom(dge, design, plot=FALSE)


baseline = 'Scramble shRNA'
coefs.shRNA = paste0('Gene',c('IFNAR2', 'IL10RB'))
res.shRNA = run_analysis( vobj.shRNA.covneg, infoSub.shRNA.covneg, baseline, coefs.shRNA, form, gsc.test)
```
...
<!---
### Using gene sets found above
--->
```{r show.zenith.CRISPRa.covNeg.previous, fig.width=8, fig.height=35, eval=FALSE}
res = rbind(res.CRISPRa$enrich, res.shRNA$enrich)

res$Coef = paste0(gsub("Gene", '', res$Coef), " (CoV-)")
res$value = with(res, delta/se)
res2 = rbind(res, resCombine[,colnames(resCombine)!="isGO"])

gs.any = unique(res2[res2$FDR < .05,]$Set)
...
## all at FDR < 10%
--->
```{r show.zenith.CRISPRa.covNeg, fig.width=8, fig.height=30,eval=FALSE}
res = rbind(res.CRISPRa$enrich, res.shRNA$enrich)
...
### Enrichments for CoV- samples
Using only genesets from above
```{r zenith.covNEg, fig.width=7, fig.height=15}

resCombine.covneg = rbind(res.CRISPRa$enrich, res.shRNA$enrich)
resCombine.covneg$Coef = factor(gsub("Gene", '', resCombine.covneg$Coef))
resCombine.covneg$value = with(resCombine.covneg, delta/se)
# storeData( 'resCombine', "Gene set analysis of differential expression" )
resCombine.covneg$isGO = FALSE
resCombine.covneg$isGO[grep("GO", resCombine.covneg$Set)] = TRUE


res = resCombine.covneg[resCombine.covneg$Set %in% attr(figSigGO, "row.names"),]
res$Set = factor( res$Set, attr(figSigGO, "row.names"))

# order based on previous figure
res = res[order(as.numeric(res$Set)),]

# get max value
ids = c(attr(figSigGO, "row.names"), attr(figSigOther, "row.names"))
zlim = max(abs(resCombine.covneg[resCombine.covneg$Set %in% ids,]$value))
zlim = c(-zlim, zlim)
...
res %>% 
  ...
  mutate(Coef = factor(Coef, newLvls)) %>%   
  dplyr::filter(isGO) %>%   
  gheatmap('Set', 'Coef', label="t-statistic", sort.col=FALSE, 
    sort.row=FALSE, zlim=zlim) 
```


### shRNA, CoV-
```{r zenith.covNEg.not.go, fig.width=8, fig.height=15}
res = resCombine.covneg[resCombine.covneg$Set %in% attr(figSigOther, "row.names"),]
res$Set = factor( res$Set, attr(figSigOther, "row.names"))
...
While IL10RB expression appears to be correlated with SARS.CoV.2 level, this association goes away 
...
form = ~ SARS.CoV.2 + MDS1 + MDS2 + RIN 
design = model.matrix(form, infoSub)
vobj = voom(dge, design, plot=FALSE)

baseline = 'Scramble CRISPRa'
coefs.CRISPRa = 'SARS.CoV.2'
res = run_analysis( vobj, infoSub, baseline, coefs.CRISPRa,  form, gsc.test)
...
Using formula: `r paste0(as.character(form), collapse=' ')`
...
tab %>% filter(Symbol %in% c("IL10RB", 'IFNAR2')) %>% 
  kable(caption="Test IL10RB, IFNAR2 versus viral load") %>% 
  kable_styling(full_width=FALSE)
```

```{r IL10RB_SARS}
ensGene = tab %>% filter(Symbol == "IL10RB") %>% rownames

df = merge(infoSub, data.frame(ID = colnames(vobj), IL10RB.expression =  t(t(vobj$E[ensGene,]))), by="ID")
df$resid = residuals(lm(IL10RB.expression ~ MDS1 + MDS2 + RIN, df))

ggplot(df, aes(resid, SARS.CoV.2)) + geom_point() + geom_smooth(method='lm') + theme_classic() + theme(aspect.ratio=1)

with(df, cor.test(resid, SARS.CoV.2))
```

## no covariates
```{r test.SARS.CoV.2.no_Cov}
form = ~ SARS.CoV.2
design = model.matrix(form, infoSub)
vobj = voom(dge, design, plot=FALSE)
...
```{r IL10RB_SARS.no_cov}
ensGene = tab %>% filter(Symbol=="IL10RB") %>% rownames

df = merge(infoSub, data.frame(ID = colnames(vobj), IL10RB.expression =  t(t(vobj$E[ensGene,]))), by="ID")

ggplot(df, aes(IL10RB.expression, SARS.CoV.2)) + geom_point() + geom_smooth(method='lm') + theme_classic() + theme(aspect.ratio=1)

with(df, cor.test(IL10RB.expression, SARS.CoV.2))
```
